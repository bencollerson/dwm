...
enum { SchemeNorm, SchemeSel, SchemeWarn, SchemeUrgent, SchemeSelWarn }; /* color schemes */
...
applyrules(Client *c)
{
	...
	c->tags = c->tags ? c->tags : c->mon->tagset[c->mon->seltags];
}
...
focus(Client *c)
{
	...
	if (c) {
		...
		attachstack(c);
		grabbuttons(c, 1);
		XSetWindowBorder(dpy, c->win, scheme[SchemeWarn][ColFg].pixel);
		setfocus(c);
	} else {
...
grid(Monitor *m) {
	...
	for(n = 0, c = nexttiled(m->cl->clients, m); c; c = nexttiled(c->next, m))
		n++;
...
setup(void)
{
	...
	for (i = 0; i < LENGTH(colors); i++)
		scheme[i] = drw_scm_create(drw, colors[i], colorcount);
	/* init bars */
	updatebars();
	updatestatus();
...
zoom(const Arg *arg)
{
	...
	if (!selmon->lt[selmon->sellt]->arrange || !c || c->isfloating)
		return;
	if (c == nexttiled(selmon->cl->clients, selmon) && (!c || !(c = nexttiled(c->next, selmon))))
		return;
	pop(c);
}
